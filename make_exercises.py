...
import json, datetime, random, sys, time
...
def time_exercise(duration, testing=testing):
...
def schedule():
    ...
    for exercise in ex_list:
        ...
        if exercise != 'chords' and exercise != 'triads':
            ...
            time_exercise(ex_list[exercise])
        else:
            time_division  = ex_list[exercise]/len(data[exercise])
            for interval in data[exercise]:
                print("Current %s to practice is %s" % (exercise,', '.join(interval)))
                time_exercise(time_division)
